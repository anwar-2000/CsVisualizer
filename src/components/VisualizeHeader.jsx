...
function VisualizeHeader() {
  ...
  return <div className={classes.VisualizeHeader}>
  <h4 className={classes.title}>{file.name}  <BsDownload size={20} style={{cursor : "pointer"}}/> </h4>
  <TfiSave size={20} color="black" style={{cursor : "pointer"}} onClick={handleSaveNewFile}/>
  <a href="" ref={downloadLink} className={{display:"hidden"}}/>
  <button><IoIosAdd color='green' /> Row</button>
  <button><MdOutlineDeleteOutline color='red' /> Row</button>
  <button><IoIosAdd color='green'/> Column</button>
  <button><MdOutlineDeleteOutline color='red'/> Column</button>
</div>
}
